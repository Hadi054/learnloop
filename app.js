...
/* ---------- CHAPTER PRACTICE (one page, checked at the end) ----------
   ONE renderer for both tracks. These were two copies of the same screen, and
   copies drift: the rating row's element ids diverged (r0..r5 against ur0..ur5)
   and Finish silently did nothing, because the handler threw on a null element.
   Everything track-specific now lives in a descriptor, so a change lands in
   both places or neither.

   The page shows the problem, all three MCQs and the written answer at once,
   answerable in any order. Nothing is revealed until Check, which grades every
   question together and then asks for the self-rating. It fills the same
   session fields the stepwise flow does, so finishLoop()/surFinish() — score,
   1/3/7/21 ladder, history append, result screen — are reused untouched. */
const PRACTICE = {
  machine: {
    get s(){ return sess; },
    item: () => sess.loop,
    pos:  () => loopPos(sess.loop),
    mark: p  => blockMark(p.b),
    rid:  n  => "r" + n,
    on: {pick:"pick", show:"showSolution", check:"check", rate:"rate", finish:"finishLoop"}
  },
  surface: {
    get s(){ return usess; },
    item: () => usess.u,
    pos:  () => surPos(usess.u),
    mark: p  => surBlockMark(p.b),
    rid:  n  => "ur" + n,
    on: {pick:"surPick", show:"surShowSolution", check:"surCheck", rate:"surRate", finish:"surFinish"}
  }
};
function practiceScreen(t){
  const ss = t.s, item = t.item(), p = t.pos(), on = t.on;
  const qs = item.assess.sets[ss.set] || item.assess.sets[0];
  const ck = ss.checked, ex = item.exercise;
  const cards = qs.map((q, qi) => {
    const pick = (ss.picks || [])[qi];
    const opts = q.options.map((o, oi) => {
      let cls = "opt";
      if(ck) cls += oi === q.correct ? " correct" : (oi === pick ? " wrong" : " faded");
      else if(oi === pick) cls += " sel";
      return `<button class="${cls}" ${ck?"disabled":""} onclick="${on.pick}(${qi},${oi})">${fmtInline(o)}</button>`;
    }).join("");
  ...
  screen(`
    <div class="eyebrow">${hexOf(p.i)} <span class="dim">// ${t.mark(p)} &middot; practice</span></div>
    <h1>${esc(item.title)}</h1>
    <div class="card">
      ${labelRow('<div class="layer-label amb">Predict it <span class="layer-note">&mdash; before you read on</span></div>')}
      ${zone(fmt(ex.prompt), item.id, "exercise.prompt", true)}
      ${zone(codeblock(ex.code), item.id, "exercise.code")}
      ${ss.shown
        ? zone(fmt(ex.solution), item.id, "exercise.solution", true)
          + `<div class="feedback good">${fmt(ex.explanation)}</div>`
        : `<button onclick="${on.show}()">Show the answer</button>`}
    </div>
    ${cards}
    <div class="card">
      ${labelRow('<div class="layer-label">Explain it <span class="layer-note">&mdash; in your own English</span></div>')}
      ${zone(fmt(item.assess.explainPrompt), item.id, "assess.explainPrompt", true)}
      <textarea id="ans" ${ck?"readonly":""} placeholder="Write it as if answering an interviewer&hellip;">${esc(ss.draftAnswer||"")}</textarea>
      ${ck ? `${labelRow('<div class="layer-label amb">Model answer</div>')}
        ${zone(fmt(item.assess.modelAnswer), item.id, "assess.modelAnswer", true)}
...
function practiceSaveDraft(ss){ const t = document.getElementById("ans"); if(t) ss.draftAnswer = t.value; }
function practicePick(ss, qi, oi){
  ss.picks = ss.picks || [];
  ss.picks[qi] = oi;
  document.querySelectorAll("#q"+qi+" .opt").forEach((b,j)=> b.classList.toggle("sel", j===oi));
}
function practiceShow(ss, redraw){ practiceSaveDraft(ss); ss.shown = true; redraw(); }
function practiceCheck(ss, item, redraw){
  practiceSaveDraft(ss);
  const qs = item.assess.sets[ss.set] || item.assess.sets[0];
  ss.correct = qs.reduce((n,q,i)=> n + ((ss.picks||[])[i] === q.correct ? 1 : 0), 0);
  ss.checked = true; ss.shown = true;
  redraw();
}
/* thin per-track entry points — the onclick strings above name these */
function practice(){ practiceScreen(PRACTICE.machine); }
function pick(qi, oi){ practicePick(sess, qi, oi); }
function showSolution(){ practiceShow(sess, practice); }
function check(){ practiceCheck(sess, sess.loop, practice); }
function goSurPractice(){ navGo(surPractice); }
function surPractice(){ surRedraw = surPractice; practiceScreen(PRACTICE.surface); }
function surPick(qi, oi){ practicePick(usess, qi, oi); }
function surShowSolution(){ practiceShow(usess, surPractice); }
function surCheck(){ practiceCheck(usess, usess.u, surPractice); }
