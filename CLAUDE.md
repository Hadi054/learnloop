...
| 7 | "Protocols on the Wire": protobuf wire format, .proto evolution, HTTP/2 frames/streams, anatomy of a gRPC call, deadlines+cancellation, streaming RPCs (+flow control), interceptors, SwiftNIO/transports, TLS+pinning, connection lifecycle, auth over gRPC | 11 | IN PROGRESS (b7-01..03 done; b7-03 verified via raw-socket python frame capture of curl --http2-prior-knowledge + ALPN/stream observation on a live host) |
