...
| 11 | "Shipping the Machine": targets/schemes/xcconfig, code signing, SPM/CocoaPods internals, scenes lifecycle, dSYMs+crash symbolication, CI for iOS, update strategy | 7 | IN PROGRESS (b11-01 done 2026-07-19; executed: swiftc -D DEV/STG/PRD → 3 binaries 3 URLs from one source, #error guard verbatim on unset env, strings-check proof PRD binary lacks staging URL (dead branches stripped), xcodebuild -showBuildSettings -xcconfig overlay materializing user-defined API_BASE_URL + $(inherited) splice + the $() //-escape trick) |
